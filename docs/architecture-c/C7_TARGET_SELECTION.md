...
| Failure boundary | `chain.proceed()` is called first and its `Throwable` is captured. The custom effect runs afterwards **regardless** of whether `chain.proceed()` threw. Any `Throwable` from the custom effect is logged/swallowed. Finally, the original captured `Throwable` is rethrown by `throwOrReturn`. If `chain.proceed()` succeeded and the custom effect fails, the original result is still returned. |
| Fatal behavior | `throwOrReturn` rethrows the captured `Throwable`, including `OutOfMemoryError` / `ThreadDeath` / `VirtualMachineError`. The second `try` block's `catch (t: Throwable)` would log an `OutOfMemoryError` from the custom effect, but `throwOrReturn` then rethrows the original captured `Throwable`. If `chain.proceed()` itself throws `OutOfMemoryError`, it is rethrown after the custom effect. |
...
| Failure boundary | `chain.proceed()` is called first and its `Throwable` captured. The custom haptic work runs afterwards. Haptic work `Throwable`s are logged/swallowed. The original `Throwable` is rethrown by `throwOrReturn`. |
| Fatal behavior | `throwOrReturn` rethrows the original captured `Throwable`, including fatal errors. No `FatalErrors.rethrowIfFatal` call. |
...
| `AutoDismissExpandedPopupsHook` | `intercept` (two surfaces) | **0** | none | after (`proceed` first) | two callbacks; `Handler` scheduling; original `Throwable` rethrown after custom effect | low | high (two classes, `Handler` semantics) | low (dominant cost is `Handler` scheduling, not reflection) | 4 |
| `QSHapticHook` | `intercept` | 2 reads when active | `HapticFeedbackUtil` per active click | after (`proceed` first) | single `try`; haptic work swallowed | low (vibration dominates) | moderate | low (vibration call dominates; reflection is a tiny fraction) | 5 |
...
4. **Compatibility surfaces are non-trivial.** Each candidate depends on ROM field/method names, possible subclasses, and observable state mutation. The two-surface `AutoDismissExpandedPopupsHook` and the list-mutation `BetterPopupsAllowFloatHook` have the largest surfaces.
