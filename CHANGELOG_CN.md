...
- 设置导航改为按需生成分类页面与搜索索引，减少首屏 Preference 创建量，同时消除页面切换点击残影并重新整理“杂项”结构。
- 移除桌面手势、充电提示和手机管家 Dock 热路径中的重复调用栈扫描，改用有界的进程内路由并补充初始化指标，不把额外工作转移到高频路径。
- 修复 AudioVisualizer 与电池指示器 Observer 所有权、启动激活状态竞态、状态栏 Insets/窗口几何同步，以及备份恢复后失效应用仍被计数的问题。
- 输入法分类改用宽泛命名，Gboard 专属底部间距功能保持明确标注，避免对不同 ROM 造成误解。
- 新增 HyperOS 充电、静音、勿扰顶部胶囊控制并标记为**开发中**；已触发真实充电灵动额头，但高度匹配未通过实机验收，不宣称已验证可用。

### 验证边界

- 统一静态规则、不变量、Python 测试、Android JVM 测试、编译与 lint 已通过；正式 APK 在发布阶段另行完成产物检查。
- 实机已确认 HyperOS StrongToast 真实调用路径，模拟测试后已恢复电池状态；灵动额头高度匹配仍待后续验证。
