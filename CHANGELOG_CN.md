...
- 电池详情在「仅充电时显示」且系统暂时拿不到电池状态时，不再误显示。

### 桌面与最近任务

- 关闭文件夹背景模糊后，在文件夹内拖动应用不再闪出系统默认模糊，松手后也保持清晰。
- 「禁用壁纸缩放」改为直接拦截桌面实际下发的壁纸 zoom 调用，最近任务和应用打开/关闭时不再缩放，不再只依赖可能被内联的 `ZOOM_ENABLED` 标志。
- 「启动器 → 其它」与「系统 → 最近任务」两处开关已区分标题和说明；任开一个即可关闭最近任务/应用过渡的壁纸缩放，仅启动器开关额外关闭解锁壁纸缩放和最近任务压暗。
- 最近任务背景模糊调到 0% 时，全面屏手势进入也会生效。该选项只控制桌面最近任务模糊，与「系统 → 其它 → 关闭窗口级模糊」互不影响；打开文件夹时仍用文件夹自己的模糊设置。

### 修复

- 动态岛上滑收回手势更稳定。
- 设备信息刷新不再依赖网速控制器槽位，温度文字在无控制器时也能显示。
- 「连接电源时不唤醒」选项 2 在 A14 上恢复 POWER / PLUGGED / RAPID / WIRELESS 的预期行为。
