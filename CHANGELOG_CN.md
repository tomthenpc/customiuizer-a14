...
- 灵动额头新增动态岛模式：基于 HyperOS 原有顶部胶囊变形，支持按挖孔 / 摄像头区域定位与垂直微调，保留系统动画，并在显示期间平滑隐藏状态栏内容。
- USB 默认用途可选跟随系统、仅充电、文件传输（MTP）或照片传输（PTP）。
- 最近任务支持隐藏应用名称；全面屏手势支持隐藏键盘关闭按钮。
- 支持关闭桌面文件夹背景模糊及部分窗口级模糊。
- 音量面板支持隐藏勿扰 / 静音快捷按钮并调整模式按钮颜色。
- 支持自动关闭位置与通知权限请求弹窗（不自动授权）。
- 新增小米更新服务、MIUI daemon、统计、手机管家营销与杀毒入口等精简控制。

### 设置与数据

- 设置首页、二级页面与搜索按功能语义重新整理，语言设置移至首页，关于页独立。
- 修复搜索进入页面后部分开关视觉状态不同步的问题，并完善页面导航生命周期。
- 备份升级为 V2 类型化格式，支持旧备份迁移、完整性校验、无效项清理及恢复失败回滚。

### 稳定性与兼容性

- 完善静态作用域与动态应用作用域处理，在支持的 LSPosed API 上按需请求目标应用作用域。
- 完善 SystemUI、桌面及系统 Hook 的生命周期、失败边界与资源释放。
- 修复侧边栏展开 Receiver 在 View 重建后的所有权与释放问题。
- 完善状态栏高度、音量百分比、USB 插入默认用途等运行时行为。

### 性能与工程

- 减少状态栏、电池、时钟、图标、通知等高频路径中的重复解析、Binder 调用与临时分配。
- Preference 更新改为按类型读取单项值，避免普通变更复制完整偏好表。
- 正式版诊断与可重建缓存进一步收敛，降低不必要常驻数据。
- 移除已废弃实验代码，整理项目文档、测试与工具链，并完善 GitHub Actions 自动验证。
