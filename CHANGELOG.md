...
发布日期：2026-07-25。状态：**已合并到 r14.7.4（不再单独发布）**。
...
## r14.2.9 — r14.2.x 系列合并整理

发布日期：2026-07-23。状态：**稳定版（r14.2 系列唯一发布，完整重启验证通过）**。

> 本版本合并 r14.2.1–r14.2.8 的全部热路径优化、生命周期治理、无效 Hook/监听清理与绘制缓存，作为 r14.2 系列唯一稳定发布。r14.2.0、r14.2.4、r14.2.7 的历史内容已整合到本版本中。
...
## r14.2.7 — 功能关闭零成本与生命周期治理（已合并到 r14.2.9）

发布日期：2026-07-23。状态：**已合并到 r14.2.9（不再单独发布）**。
...
## r14.2.4 — 关闭时零成本与接收者防重注册（合并 r14.2.1-r14.2.4，已合并到 r14.2.9）
...
## r14.2.0 — Hook 热路径与调度架构优化（已合并到 r14.2.9）
