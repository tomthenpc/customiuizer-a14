...
| Hook 接口 | 稳定版：libxposed API 101；r14.11.0 候选版：min API 101 / target API 102 |
...
- **r14.11.0 当前是 API 101 / API 102 双兼容优化测试候选版**：在 r14.10.0 双兼容基线上完成 Kotlin 正确性、生命周期与热路径收尾；未启用 Hot Reload。完成 API 101 与 API 102 两套实机冷启动和关键功能验证前，不替代现有稳定版。
